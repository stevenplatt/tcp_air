...
Caveat: self-signed cert is added. valid cert check is set to false in resolve_conflicts methods. 
This will be fixed in a final version.

Added: SSL support, /nodes/remove flask method
...
"""
app = initializes an instance of a flask application
node_identifier creates a random ID for the our node
blockchain = calls the main blockchain object above to initiate the chain
/mine allow GET request to mine a new block
/transactions/new allows POST request to add new transaction
/chain allows GET request to return a full chain
app.run declares the port for running the flask application, 0.0.0.0 binds all interfaces
"""
...
def new_transaction():
    ...
    required = ['sender', 'recipient', 'mac', 'action', 'amount']
    if not all(k in values for k in required):
        return 'Missing values', 400

    # Create a new Transaction
    index = blockchain.new_transaction(values['sender'], values['recipient'], values['mac'], values['action'], values['amount'])

    response = {'message': f'Transaction will be added to Block {index}'}
    
    new_device = values.get('mac')

    subprocess.call('uci add firewall rule', shell=True)
    subprocess.call("uci set firewall.@rule[-1].target='ACCEPT'", shell=True)
    subprocess.call("uci set firewall.@rule[-1].proto='tcp udp icmp'", shell=True)
    subprocess.call("uci set firewall.@rule[-1].src='lan'", shell=True)
    subprocess.call(['uci set firewall.@rule[-1].src_mac=' + "'{0}'".format(new_device)], shell=True)
    subprocess.call('uci commit && service firewall reload', shell=True)
